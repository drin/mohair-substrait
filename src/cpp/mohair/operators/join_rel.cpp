...
// >> Implementations for SubstraitOpImpl<JoinRel>

namespace mohair {

  //! Copy the internal Rel and RelOp without recursing into inputs
  template <>
  unique_ptr<Rel> SubstraitOpImpl<JoinRel>::CopyRelOp() {
    unique_ptr<Rel> rel_copy  { std::make_unique<Rel>() };
    JoinRel&        join_copy = *(rel_copy->mutable_join());

    join_copy.mutable_common()->CopyFrom(rel_op->common());
    join_copy.mutable_advanced_extension()->CopyFrom(rel_op->advanced_extension());

    join_copy.mutable_expressions()->CopyFrom(rel_op->expressions());
    join_copy.mutable_post_join_filter()->CopyFrom(rel_op->post_join_filter());
    join_copy.set_type(rel_op->type());
  ...
  template <>
  void
  SubstraitOpImpl<JoinRel>::AddToPipeline( PlanPipeline&  plan_pipe
  ...
  template <>
  Rel* SubstraitOpImpl<JoinRel>::MoveToRel(Rel* new_srel) {
    // Cache the pointer to the old rel
    Rel* old_srel = rel;
    
    // Move rel_op into the new rel and refresh rel_op (in case its invalidated)
    new_srel->set_allocated_join(old_srel->release_join());
    rel    = new_srel;
    rel_op = new_srel->mutable_join();
