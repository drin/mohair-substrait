...
// >> Implementations for SubstraitOpImpl<CrossRel>

namespace mohair {

  //! Copy the internal Rel and RelOp without recursing into inputs
  template <>
  unique_ptr<Rel> SubstraitOpImpl<CrossRel>::CopyRelOp() {
    unique_ptr<Rel> rel_copy   { std::make_unique<Rel>() };
    CrossRel&       cross_copy = *(rel_copy->mutable_cross());

    cross_copy.mutable_common()->CopyFrom(rel_op->common());
    cross_copy.mutable_advanced_extension()->CopyFrom(rel_op->advanced_extension());
  ...
  template <>
  void
  SubstraitOpImpl<CrossRel>::AddToPipeline( PlanPipeline&  plan_pipe
  ...
  template <>
  Rel* SubstraitOpImpl<CrossRel>::MoveToRel(Rel* new_srel) {
    // Cache the pointer to the old rel
    Rel* old_srel = rel;
    
    // Move rel_op into the new rel and refresh rel_op (in case its invalidated)
    new_srel->set_allocated_cross(old_srel->release_cross());
    rel    = new_srel;
    rel_op = new_srel->mutable_cross();
