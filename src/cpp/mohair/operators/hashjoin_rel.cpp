...
// >> Implementations for SubstraitOpImpl<HashJoinRel>

namespace mohair {

  //! Copy the internal Rel and RelOp without recursing into inputs
  template <>
  unique_ptr<Rel> SubstraitOpImpl<HashJoinRel>::CopyRelOp() {
    unique_ptr<Rel> rel_copy   { std::make_unique<Rel>() };
    HashJoinRel&    hjoin_copy = *(rel_copy->mutable_hash_join());

    hjoin_copy.mutable_common()->CopyFrom(rel_op->common());
    hjoin_copy.mutable_advanced_extension()->CopyFrom(rel_op->advanced_extension());

    hjoin_copy.mutable_post_join_filter->CopyFrom(rel_op->post_join_filter());
    hjoin_copy.set_type(rel_op->type());

    if (hjoin_copy.has_left_keys()) {
      hjoin_copy.mutable_left_keys()->CopyFrom(rel_op->left_keys());
    }

    if (hjoin_copy.has_right_keys()) {
      hjoin_copy.mutable_right_keys()->CopyFrom(rel_op->right_keys());
    }

    if (hjoin_copy.has_keys()) { hjoin_copy.mutable_keys()->CopyFrom(rel_op->keys()); }
  ...
  template <>
  void
  SubstraitOpImpl<HashJoinRel>::AddToPipeline( PlanPipeline&  plan_pipe
  ...
  template <>
  Rel* SubstraitOpImpl<HashJoinRel>::MoveToRel(Rel* new_srel) {
    // Cache the pointer to the old rel
    Rel* old_srel = rel;
    
    // Move rel_op into the new rel and refresh rel_op (in case its invalidated)
    new_srel->set_allocated_hash_join(old_srel->release_hash_join());
    rel    = new_srel;
    rel_op = new_srel->mutable_hash_join();
